...
function toShellExample (state, endpoint) {
  return `curl -H"Accept: application/vnd.github.v3+json" ${state.baseUrl}${urlTemplate.parse(endpoint.path).expand(PARAMETER_EXAMPLES)}`
}

function toJsExample (state, endpoint) {
  return `octokit.issues.get({${endpoint.params.map(param => `${param.name}: ${JSON.stringify(PARAMETER_EXAMPLES[param.name])}`).join(', ')}})`
}
...
async function checkOrUpdateRoutes (options) {
  ...
  allEndpoints.forEach(async endpoint => {
    ...
    const [scope] = endpoint.documentationUrl.substr(state.baseUrl.length + 1).split('/')
